...
fn main() {
    App::new()
        .insert_resource(Time::<Fixed>::from_seconds(1.0/5.0))
        .add_plugins(DefaultPlugins)
        .add_systems(Startup, setup)
        .add_systems(Update, (cube_rotator_system, rotator_system))
        .add_systems(FixedUpdate, screenshot_system)
        .run();
}
...
fn setup(
    ...
    commands.insert_resource(TheImage {handle: image_handle.clone()});
}

fn screenshot_system(
    mut commands: Commands, 
    texture_resource: ResMut<TheImage>,
    mut counter: Local<u32>
) {
    let sc = Screenshot::image(texture_resource.handle.clone());
